 ...
 */
static void free_eps(endpoint eps[], int num)
{
	int i;

	for (i = 0; i < num; i++) {
		mem_free(eps[i].uaddr, 0);
		mem_free(eps[i].proto, 0);
		mem_free(eps[i].family, 0);
	}
	return;
}
 ...
 */
static nis_server *get_server(struct sockaddr_in *sin, char *host,	/* name of the time host */
 ...
 * td = "server" - "client"
 */
int __rpc_get_time_offset(struct timeval *td,	/* Time difference */
			  ...
			  /* known network identifier */ )
{
	...
	*useep = NULL;		/* endpoint of xp */
	...
	time_valid = 0;
	status =
	    clnt_call(clnt, auth, RPCBPROC_GETTIME, (xdrproc_t) xdr_void, NULL,
 ...
 error:
	...
	if (time_valid) {
		...
		delta =
		    (thetime >
		     tv.tv_sec) ? thetime - tv.tv_sec : tv.tv_sec - thetime;
