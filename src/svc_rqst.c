...
int svc_rqst_free_evchan(uint32_t chan_id, uint32_t flags)
{
    ...
    /* XXX to simplify life for consumers, allow them to unregister
     * all the xprt associations here */
    n = opr_rbtree_lookup(&svc_rqst_set_.t, &trec.node_k);
    if (n) {
        sr_rec = opr_containerof(n, struct svc_rqst_rec, node_k);
